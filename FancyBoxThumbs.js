...
$("a.image").each(function(){
	var img_src_parts, img_filename, img_path, new_img_src, orig_img_src = $(this).find('img').attr("src");
	...
	if(!$.inArray("thumb",img_src_parts)>0) {
		...
		$(this).attr("href", orig_img_src).attr("rel", "group");
		return;
	}
	...
	$(this).attr("href", new_img_src).attr("rel", "group");

//now set fancybox on all thumbnail links
}).fancybox({"transitionIn":"elastic","transitionOut":"elastic","titlePosition":"inside"});